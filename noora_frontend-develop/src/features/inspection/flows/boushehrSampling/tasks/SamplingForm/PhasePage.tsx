		...
		if (hooks.get().length === 0) {
			hooks.registerHook("pre-submit", async ({ data }) => {
				...
				if (data.ProductsData.length) {
					// const sealNoArray = (await data.ProductsData[0].sealNo)
					//   ? data.ProductsData[0].sealNo?.split(",")
					//   : [];
					// const smallLabelNoArray = (await data.ProductsData[0].smallLabelNo)
					//   ? data.ProductsData[0].smallLabelNo.split(",")
					//   : [];
					// const bigLabelNoArray = (await data.ProductsData[0].bigLabelNo)
					//   ? data.ProductsData[0].bigLabelNo.split(",")
					//   : [];
					// const codes = await sealNoArray?.concat(
					//   smallLabelNoArray,
					//   bigLabelNoArray
					// );
					// await PutProductsHistory({
					//   usedCodes: codes,
					//   id: fields.BranchId,
					// });
				}
