...
const schema = z.object({
	...
	[ids.informationReviewByCeoStatus]: z.string(),
	[ids.informationReviewByCeoNote]: z.string(),
...
function PhasePage() {
	...
	const { [ids.informationReviewByCeoStatus]: reviewStatus } = watch();
		...
		hooks.registerHook(
			...
			async ({ task, data }: { task: Task; data: FormData }) => {
				...
				data[ids.previousTask] = {
					...
					noteContent:
						data[ids.informationReviewByCeoStatus] === ReviewStatus.Forward
							? ""
							: data[ids.informationReviewByCeoNote],
					noteType:
						data[ids.informationReviewByCeoStatus] === ReviewStatus.Forward
							? "info"
							: "danger",
		...
		hooks.registerHook(
			...
			async ({ task, data }: { task: Task; data: FormData }) => {
				...
				await setStageOfInstance(
					task.instanceId,
					data[ids.informationReviewByCeoStatus] === ReviewStatus.Forward
						? data[ids.hasAccountingElements]
							? "information-review-by-accountant"
							: "cancellation-apply"
						: data[ids.informationReviewByCeoStatus] ===
	...
	return (
		<div className="grid grid-cols-12 gap-x-4 gap-y-6 md:gap-x-10">
			...
			<InspectionCancellationNote creator manager />

			<Separator className="col-span-full h-1" />

			<FormField
				control={control}
				name={ids.informationReviewByCeoStatus}
				render={({ field }) => (
					<FormItem className="col-span-full !col-start-1 xs:col-span-9 sm:col-span-7 md:col-span-5 xl:col-span-3">
			...
			<FormField
				control={control}
				name={ids.informationReviewByCeoNote}
				render={({ field }) => (
