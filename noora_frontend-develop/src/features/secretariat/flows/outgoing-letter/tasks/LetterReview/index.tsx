...
import { reviewerPositionOptions } from "../../data/reviewerPositionOptions";
import {
...
const schema = z.object({
	...
	[ids.letterReviewStatus]: z.custom<ReviewStatus>(),
	[ids.letterReviewNote]: z.string(),
...
function PhasePage() {
	...
	const {
		...
		[ids.reviewBy]: reviewBy,
		[ids.letterReviewStatus]: reviewStatus,
	} = fields;

	const isPositiveStatus =
		typeof reviewStatus === "undefined" ||
		reviewStatus === ReviewStatus.Forward ||
		reviewStatus === ReviewStatus.Reassign;
		...
		hooks.registerHook(
			...
			async ({ task, data }: { task: Task; data: FormSchema }) => {
				...
				data[ids.previousTask] = {
					...
					assigneeTitle: assigneesTemplate[AssigneeType.Reviewer],
					noteContent: data[ids.letterReviewNote],
					noteType:
						data[ids.letterReviewStatus] === ReviewStatus.Return
							? "danger"
							: "info",
				};
				...
				if (data[ids.letterReviewStatus] === ReviewStatus.Reassign) {
					// set reviewBy
	...
	return (
		<div className="grid grid-cols-12 gap-x-4 gap-y-6 md:gap-x-10">
			<FormItem className="col-span-full !col-start-1 xs:col-span-9 sm:col-span-7 md:col-span-5 xl:col-span-3">
				<FormLabel>ایجاد کننده</FormLabel>
				<FormControl>
					<Input disabled value={assignees[AssigneeType.Creator]!.name} />
				</FormControl>
			</FormItem>
			...
			// name={
			// 	task.data[ids.letterReviewStatus] === ReviewStatus.Reassign
			// 		? reviewBy.at(reviewBy.length - 2)?.name
			// 		: undefined
			// }
			/>
			...
			<div className="col-span-full space-y-4">
				...
				<LetterDownloadButton
					values={{
						...fields,
						reviewer: task.data[ids.assignees][AssigneeType.Reviewer],
					}}
				/>
			</div>
					...
					<FormItem className="col-span-full !col-start-1 xs:col-span-9 sm:col-span-7 md:col-span-5 xl:col-span-3">
						...
						<FormControl>
							<Select value={value} onValueChange={onChange} {...field}>
								...
								<SelectContent>
									{value && <SelectItem value="none">-</SelectItem>}
			...
			{reviewStatus === ReviewStatus.Reassign && <ReviewerSelect />}

			{reviewStatus === ReviewStatus.Forward && (
				<FormField
					control={control}
					name={ids.reviewerPosition}
					render={({ field: { ref, value, onChange, ...field } }) => (
						<FormItem className="col-span-full !col-start-1 xs:col-span-9 sm:col-span-7 md:col-span-5 xl:col-span-3">
							<FormLabel>
								جایگاه سازمانی تاییدکننده
			...
			<FormField
				control={control}
				name={ids.letterReviewNote}
				render={({ field }) => (
					<FormItem className="col-span-full">
						<FormLabel>
							توضیحات
							{(isNegativeStatus || reviewStatus === ReviewStatus.Reassign) && (
								<span className="text-red-600"> *</span>
							)}
						</FormLabel>
						<FormControl>
