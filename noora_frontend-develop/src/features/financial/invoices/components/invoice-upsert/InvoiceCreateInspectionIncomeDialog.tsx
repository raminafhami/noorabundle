...
const formSchema = z
	.object({
		...
		caseType: z.custom<CaseType>(),
		recipient: z.custom<InvoiceRecipient>(),
...
function InvoiceCreateInspectionIncomeDialog({
	payload: { type, recipient },
	open,
	onClose,
}: DialogProps<
	{ type?: InvoiceType; recipient?: InvoiceRecipient },
	| {
			type: InvoiceType;
...
>) {
	...
	async function handleSubmit(values: FormSchema) {
		...
		onClose({
			type: values.caseType as unknown as InvoiceType,
					...
					>
						...
						>
							<div className="grid grid-cols-12 gap-6">
								<InstanceSelect type={type} recipientId={recipient?.refId} />
...
function InstanceSelect({
	...
	const handleSelect = useCallback(
		async (_: unknown, instanceId: string) => {
			setSearchTerm("");
			setIsOpen(false);
			...
			if (type && type !== caseType) {
				setError("caseNo", {
					message: "درخواست مورد نظر از لحاظ نوع درخواست دارای مغایرت است.",
				});
				return;
			}
			...
			if (recipientId && recipientId !== recipient.refId) {
				setError("caseNo", {
					message:
		...
		[instances, type, recipientId, clearErrors, setError, setValue],
	);
...
export { InvoiceCreateInspectionIncomeDialog };
