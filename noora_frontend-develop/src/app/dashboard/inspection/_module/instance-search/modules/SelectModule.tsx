    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild className="flex flex-col w-[30%]">
        <Button
          ...
          role="combobox"
          aria-expanded={open}
          className="w-full justify-between flex-row h-10 whitespace-pre-line rounded-2xl max-w-[400px] ">
          {value.length ? value.map((item) => item.label).join(", ") : "انتخاب"}
          <ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" />
        </Button>
      </PopoverTrigger>
      <PopoverContent className="w-full p-0 PopoverContent overflow-y-auto">
        <Command>
