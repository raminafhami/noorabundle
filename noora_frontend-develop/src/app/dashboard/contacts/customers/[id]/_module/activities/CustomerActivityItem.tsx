		...
		<Card className="cursor-default">
			<CardContent className="relative flex flex-col gap-4 py-4 pe-6 ps-8 sm:flex-row">
				<div className="space-y-2">
					<h6 className="leading-6">{activity.title}</h6>
							...
							<Tooltip>
								<TooltipTrigger asChild>
									<div className="flex max-w-fit items-center gap-2">
										...
										<span>
											{asNavigationProp(activity.buyerId).name.trim() ||
												asNavigationProp(activity.buyerId).metadata.nameEn}
										</span>
									</div>
								</TooltipTrigger>
								<TooltipContent>خریدار</TooltipContent>
							</Tooltip>
						)}
						...
						{activity.description &&
							activity.description !== activity.title && (
								<Tooltip>
									...
									<TooltipContent>توضیحات</TooltipContent>
								</Tooltip>
							)}
					</div>
				</div>
