...
function CustomersDebtsRankCard() {
		...
		(async () => {
			try {
				...
				const filters: string[] = [
					...
					"parameters.assignees.customer::exists",
				...
				const result = await getInstancesReport({
					...
					size: 100,
					filters,
					groupBy: "parameters.assignees.customer.id",
					aggFunc: "parameters.inspectionFeeInRial::sum",
					select: "parameters.assignees.customer",
				});
				...
				const data = result.data
					...
					.slice(0, 10)
					.map((x) => ({
						customer: x.select.parameters.assignees.customer,
	...
	return (
		<>
			<CardHeader orientation="horizontal">
				<CardTitle>رتبه بندی مشتریان بر اساس میزان بدهی</CardTitle>
				<CardNav>
