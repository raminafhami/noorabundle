...
interface FormData {
	[ids.contractEmployerObligations]: string[];
}

export const EmployerObligationsWidget = memo(
	function EmployerObligationsWidget(): JSX.Element {
		...
		const { [ids.contractEmployerObligations]: obligations } = fields;

		useEffect(() => {
			register(ids.contractEmployerObligations, {
				validate: (v) => {
					if (v && v.length === 0) {
		...
		useEffect(() => {
			if (obligations === undefined) {
				setValue(ids.contractEmployerObligations, [
					"کارفرما مکلف است هزینه های موضوع قرارداد را پس از انجام عملیات مربوطه و قبل از صدور گزارش بازرسی در قبال اخذ رسید به بازرس پرداخت نماید.",
				]);
			}
		...
		return (
			<div className="col-span-6 col-start-1 space-y-2">
				...
				<FieldError error={errors[ids.contractEmployerObligations]} />
			</div>
		);
	},
);
