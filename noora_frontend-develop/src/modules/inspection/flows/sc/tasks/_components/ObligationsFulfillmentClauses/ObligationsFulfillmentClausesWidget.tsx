...
interface FormData {
	[ids.contractObligationsFulfillmentClauses]: string[];
}

export const ObligationsFulfillmentClausesWidget = memo(
	function ObligationsFulfillmentClausesWidget(): JSX.Element {
		...
		const { [ids.contractObligationsFulfillmentClauses]: obligations } = fields;

		useEffect(() => {
			register(ids.contractObligationsFulfillmentClauses, {
				validate: (v) => {
					if (v && v.length === 0) {
		...
		useEffect(() => {
			if (obligations === undefined) {
				setValue(ids.contractObligationsFulfillmentClauses, []);
			}
		...
		return (
			<div className="col-span-6 col-start-1 space-y-2">
				...
				<FieldError error={errors[ids.contractObligationsFulfillmentClauses]} />
			</div>
		);
	},
);
