...
import { LC_CERTIFICATE_KEYS } from "../../consts";
import { ids } from "../../models/Ids";
import { downloadCertificateWord } from "../../utils/downloadCertificateWord";
import { getCertificateTemplate } from "../../utils/getCertificateTemplate";
...
function CertificatePage() {
	...
	useEffectOnce(() => {
		const fetchData = async () => {
			...
			try {
				if (
					LC_CERTIFICATE_KEYS.filter((x) => data[x] === undefined).length !== 0
				) {
					setIsLoading(true);
					setErrorMessage(undefined);

					data = await getInstanceById(instance.id, LC_CERTIFICATE_KEYS).then(
...
function CertificatePreview() {
	...
	const LC_CERTIFICATE_TEMPLATE = getCertificateTemplate(instance);
		...
		(async () => {
			try {
				...
				const content = await generateInspectionCertificate(instance, {
					...
					template: LC_CERTIFICATE_TEMPLATE,
					qrCode,
				});
	...
	}, [instance, qrCode, LC_CERTIFICATE_TEMPLATE]);
