		...
		if (hooks.get().length === 0) {
			hooks.registerHook(
				...
				async ({ task, data }: { task: Task; data: FormData }) => {
					if (
						data[ids.certificateDraftTemplateStatus] === ReviewStatus.Forward
					) {
						// set previous task
						data[ids.previousTask] = {
							...
							noteContent: task.data[ids.certificateReviewNote],
						};
					}
				},
			);
          ...
          دانلود PDF
			...
			{task.instanceVersion >= 6 && (
				<div className="col-span-full !col-start-1 space-y-2 xs:col-span-9 sm:col-span-7 md:col-span-5 xl:col-span-3">
					<label htmlFor={ids.certificateDraftTemplateStatus}>وضعیت:</label>
					<Controller
						control={control}
						name={ids.certificateDraftTemplateStatus}
