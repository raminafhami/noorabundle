...
export function PhasePage() {
	...
	const { control, resetField, watch } = useFormContext<FormData>();

	const { [ids.certificateFormByTechExpertStatus]: reviewStatus } = watch();
		...
		if (hooks.get().length === 0) {
			hooks.registerHook(
				...
				async ({ task, data }: { task: Task; data: FormData }) => {
					...
					// set optional fields
					!data[ids.insuredBy] && (data[ids.insuredBy] = "");
					!data[ids.seller] && (data[ids.seller] = "");

					// set previous task
					if (
						data[ids.certificateFormByTechExpertStatus] === ReviewStatus.Return
					) {
						data[ids.previousTask] = {
							...
							noteContent: data[ids.certificateFormByTechExpertNote],
							noteType: "danger",
						};
					} else {
						// validate dates
			...
			hooks.registerHook("submit", async ({ task }) => {
				await addWatcherToInstance(
					task.instanceId,
					task.data[ids.assignees][AssigneeType.SeniorExpert]?.id,
				);
			});
		}
	}, [hooks]);
		...
		<div className="grid grid-cols-12 gap-x-4 gap-y-6 md:gap-x-10">
			{task.data[ids.previousTask] ? (
				...
				<Referrer
					...
					noteId={ids.certificateFormByAdminNote}
				/>
			)}
			...
			{task.instanceVersion >= 6 && (
				<div className="col-span-full !col-start-1 space-y-2 xs:col-span-9 sm:col-span-7 md:col-span-5 xl:col-span-3">
					...
					<Controller
						control={control}
						name={ids.certificateFormByTechExpertStatus}
			...
			<div className="col-span-full space-y-2">
				<label htmlFor={ids.certificateFormByTechExpertNote}>توضیحات:</label>
				<Controller
					control={control}
					name={ids.certificateFormByTechExpertNote}
					render={({ field, fieldState }) => (
						<>
