...
export class Compiler {
  ...
  compile(
    ...
    const compiledProcessDefinition = JSON.parse(
      JSON.stringify(processDefinition),
    );

    // Check if start stage is present, check for multiple stages
    const startIndex = compiledProcessDefinition.stages.findIndex(
      (stage) => stage.subType === Constants.STAGE_SUB_TYPES.START,
    );
    const endIndex = compiledProcessDefinition.stages.findIndex(
      (stage) => stage.subType === Constants.STAGE_SUB_TYPES.END,
    );
    ...
    if (startIndex !== -1) {
      compiledProcessDefinition.stages[startIndex].status =
        Constants.STAGE_STATUSES.ACTIVE; // status of start stage
      compiledProcessDefinition.stages[startIndex].timeActivated = now; // status of start stage
    }

    const stageIndexJSON = {};
    compiledProcessDefinition.processDefinitionId =
      compiledProcessDefinition.id;
    ...
    compiledProcessDefinition.processDefinitionKey =
      compiledProcessDefinition.key;
    compiledProcessDefinition.processDefinitionName =
      compiledProcessDefinition.name;
    ...
    compiledProcessDefinition.rootProcessInstanceId = rootProcessInstanceId
      ? rootProcessInstanceId
      : compiledProcessDefinition._id.valueOf();
    ...
    // }
    compiledProcessDefinition.stages = compiledProcessDefinition.stages.map(
      (task, index) => {
