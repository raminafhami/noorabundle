...
import { ProcessInstanceRepositoryImpl } from 'src/modules/process-instances/repository/process-instances.repository.impl';
import { ProcessDefinitionRepositoryImpl } from 'src/modules/process-definitions/repository/process-definitions.repository.impl';
import CustomError from 'src/common/providers/custom-error';
import { CONNECTOR_KEY, ConnectorConfig } from './connector-config';
import { DebtService } from 'src/modules/users/services/debt.service';
import { CompleteTaskBody } from '../dtos';
import { CancelProcessInstanceDto } from 'src/modules/process-instances/dtos';
import * as ms from 'ms';
import { AppDelegateService } from 'src/modules/app-delegate/app-delegate.service';
import { Server, Socket } from 'socket.io';

@Injectable()
export class Executor implements OnModuleInit {
  private readonly logger: Logger = new Logger(Executor.name);
  private executorSocket: Server;
  setSocket(server: Server) {
    this.executorSocket = server;
  }
  constructor(
    ...
    private appDelegate: AppDelegateService,
  ) {}
  onModuleInit() {
  ...
  async instanceLocator(processDefinitionKey, rootProcessInstanceId) {
    const parentProcessInstance =
      await this.processInstanceRepositoryImpl.findOne(
        { rootProcessInstanceId, processDefinitionKey },
        {},
        { $natural: -1 },
      );
   ...
   * @param path {string} - value reference expression, e.g. "$[(processDefKey).<key>.name]"
   * @returns {promise} - referenced value wrapped in promise
   */
  async valueLocator(processInstance, path: any) {
    // '$[parameters.value]'
    if (
      typeof path === 'string' &&
      path.startsWith('$[') &&
      ...
      path = path.slice(2, -1);
      if (path === 'owner') {
        return processInstance.owner;
      }
      if (path.toLowerCase().startsWith('service')) {
        const [, func] = path.split('.');
        ...
        if (_path[0] && _path[0].startsWith('(') && _path[0].endsWith(')')) {
          ...
          if (remoteDefinitionKey !== processInstance.processDefinitionKey) {
            processInstance = await this.instanceLocator(
              remoteDefinitionKey,
        ...
        _path.shift();
        const newPath = ['parameters', _path.join('.')].join('.');
  ...
  conditionEvaluator(processInstance, gateway) {
    try {
      //get the conditions
      return gateway.conditionsExp.map((condition) => {
        const data = {};
        condition.expression = condition.expression.replace(
          ...
          (match) => {
            ...
            params.shift();
            params = params.join('.');
            const path = ['parameters', params].join('.');
            const value = _.get(processInstance, path);
            console.log(path, value);
            _.set(data, params, value);
            return params;
          },
        );
        const vm = new VM({ sandbox: data });
        condition.isValid = vm.run(condition.expression);
        return condition;
      });
    } catch (err) {
      this.logger.error(err);
      return gateway.conditionsExp;
    }
  }
   ...
   */
  // todo task change new
  async ifElseHandler(processInstance, conditions, userId?: string) {
   ...
   */
  // todo task change new
  async exclusiveHandler(processInstance, stage, conditions, userId?: string) {
    // TODO validate if else condition object
    ...
    if (cond) {
      // todo task change new
      await this.goToNextStage(processInstance, cond.onTrueNextStage, userId);
    } else {
      // todo task change new
      await this.iterateNextStages(processInstance, stage.nextStages, userId); // TODO need to be changed
    }
  }
   ...
   */
  // todo task change new
  async parallelHandler(processInstance, gateway, userId?: string) {
    const sourceStages = processInstance.stages.filter((stage) =>
      stage.nextStages.includes(gateway.key),
    );
    console.log({ sourceStages });

    const completedStages = sourceStages.filter(
      (stage) =>
        stage.status === Constants.STAGE_STATUSES.COMPLETED ||
        stage.status === Constants.STAGE_STATUSES.ERROR,
    );
    ...
    if (sourceStages.length === completedStages.length) {
      for (const stageKey of gateway.nextStages) {
        // todo task change new
        await this.goToNextStage(processInstance, stageKey, userId);
      }
    }
  }
   ...
   */
  async iterateNextStages(processInstance, nextStages, userId?: string) {
    if (nextStages.length) {
      ...
      const getInstance = await this.processInstanceRepositoryImpl.findOne({
      ...
      if (processInstance.status !== Constants.STAGE_STATUSES.COMPLETED) {
        const [isValid] = this.validateCriteria(
          updated.criteria,
          updated._flags,
        );
        if (isValid) {
   ...
   * @param gateway {object} - gateway stage object
   */
  async gatewayHandler(processInstance, gateway, userId?: string) {
    // const gatewayConditions = await Promise.all(this.conditionEvaluator(processInstance, gateway));
    const gatewayConditions = this.conditionEvaluator(processInstance, gateway);
    const complete = true;
    ...
    switch (gateway.subType) {
      case Constants.STAGE_SUB_TYPES.IF_ELSE: // [DEPRICATED]
        // todo task change new
        await this.ifElseHandler(processInstance, gatewayConditions, userId);
        break;
      case Constants.STAGE_SUB_TYPES.SWITCH_CASE:
        break;
      case Constants.STAGE_SUB_TYPES.EXCLUSIVE:
        // todo task change new
        await this.exclusiveHandler(
          processInstance,
          gateway,
          gatewayConditions,
          userId,
      ...
      case Constants.STAGE_SUB_TYPES.PARALLEL:
        // todo task change new
        await this.parallelHandler(processInstance, gateway, userId);
        break;
    }
    ...
    const setValues = {
      ...
      //todo check this - remove for can use again
      // 'stages.$.conditionsExp': gatewayConditions,
    };
  ...
  async reopenProcessInstance(processInstanceId) {
    ...
    if (processInstance.isParallel) {
      ...
      const activeStages = processInstance.stages.map((_stage) => {
        ...
        if (
          stage.type === Constants.STAGE_TYPES.ACTIVITY &&
          !skipStages.includes(stage.key)
        ) {
          const history = {
            status: stage.status,
      ...
      const response = await this.makeMultipleStagesActive(
        processInstance._id,
        setValues,
      );
      // todo task change new
      await this.iterateNextStages(
        response,
        response.stages[response._startIndex].nextStages,
      );
      ...
      const response =
        await this.processInstanceRepositoryImpl.findOneAndUpdate(
          condition,
          setValues,
        );
  ...
  async createChildProcessInstance(parentProcessInstance, parentTask) {
    try {
      const defCond = {
        ...(parentTask.processDefinitionKey && {
          key: parentTask.processDefinitionKey,
        }),
        ...(parentTask.processDefinitionId && {
          _id: parentTask.processDefinitionId,
        }),
      };
      ...
      if (!workflow) {
        this.logger.error(
          `Error in creating child process instance -> Workflow does not exists for key [${parentTask.processDefinitionKey}]`,
        );
        return false;
      }
      const compiledWorkflow = this.compiler.compile(
        workflow,
        parentProcessInstance.rootProcessInstanceId,
        parentProcessInstance._id,
        parentTask._id,
      );
      const childProcessInstance =
        await this.processInstanceRepositoryImpl.create(compiledWorkflow);
      const [isValid, error, parameters] = this.validateParameters(
        parentTask.properties,
      ...
      // }
      await this.startFlow(childProcessInstance, {
        parentProcessInstanceId: parentProcessInstance._id,
        parameters: { ...parameters, ...parentProcessInstance.parameters },
      });
      this.logger.debug(
        `Child process instantiated: Parent Instance [${parentProcessInstance.name}] -> Parent Task [${parentTask.name}] -> Current Instance [${childProcessInstance.name}]`,
      );
      const condition = {
      ...
      this.logger.error(
        `Error in creating child process instance for key [${parentTask.processDefinitionKey}] -> `,
        err,
      );
    }
  }
  ...
  validateParameters(properties, parameters = {}, error = '') {
    ...
    //   if (!properties?.find((p) => p?.key == k)) {
    //     delete parameters[k];
    //   }
    // });
    for (const field of properties) {
      const fieldData = parameters[field.key];
      if (fieldData === undefined) {
        ...
        fieldData?.toString() === '[object Object]'
      ) {
        // todo check this
        // [isValid, error, parameters] = this.validateParameters(
        ...
        if (field.value.arrayOf == 'number' || 'string' || 'boolean') {
          for (let j = 0; j < fieldData.length; j++) {
            if (field.value.arrayOf !== typeof fieldData[j]) {
              [isValid, error] = [
                false,
                `[${field.key}] must be of an array of type [${field.value.arrayOf}].`,
              ];
              break;
            }
          }
        ...
        if (['upload', 'template', 'enum'].includes(field.value.type)) {
          break;
        }
        [isValid, error] = [
          false,
          `[${field.key}] must be of type [${field.value.type}].`,
        ];
        break;
      }
      if (!isValid) {
  ...
  validateCriteria(criteria, flags): [boolean, string] {
    if (criteria) {
      ...
      if (
        criteria.allCompleted !== undefined &&
        criteria.allCompleted &&
        flags?._allCompleted
      ) {
        status = Constants.STAGE_STATUSES.COMPLETED;
        return [bool, status];
      ...
      if (
        criteria.anyCompleted !== undefined &&
        criteria.anyCompleted &&
        flags?._anyCompleted
      ) {
        status = Constants.STAGE_STATUSES.COMPLETED;
        return [bool, status];
      ...
      if (
        criteria.allActivitiesCompleted !== undefined &&
        criteria.allActivitiesCompleted &&
        flags?._allActivitiesCompleted
      ) {
        status = Constants.STAGE_STATUSES.COMPLETED;
        return [bool, status];
      }
      ...
      if (
        criteria.anyActivitiesCompleted !== undefined &&
        criteria.anyActivitiesCompleted &&
        flags?._anyActivitiesCompleted
      ) {
        status = Constants.STAGE_STATUSES.COMPLETED;
        return [bool, status];
      }
      if (
        criteria.mandatoryCompleted !== undefined &&
        criteria.mandatoryCompleted &&
        flags?._mandatoryCompleted
      ) {
        status = Constants.STAGE_STATUSES.COMPLETED;
        return [bool, status];
      }
  ...
  async callConnector(connector, parameters) {
    ...
    if (!connector.config.connectorKey) {
      switch (connector?.type) {
        case 'rest':
          if (
            connector?.config.method?.toLowerCase() === 'post' ||
            connector?.config.method?.toLowerCase() === 'put' ||
            connector?.config.method?.toLowerCase() === 'patch'
          ) {
      ...
      switch (connector.config.connectorKey) {
        case CONNECTOR_KEY.SEND_SMS:
          const config = this.connectorConfig.getSmsConfig(
            connector.config,
            parameters,
          );
          [err, data] = await this.http.call(config);
          console.log(err, data);
          break;
        case CONNECTOR_KEY.BANK_GATEWAY_TOKEN:
          const bankConfig = this.connectorConfig.getBankGatewayTokenConfig({
            amount: parameters.amount,
            transactionType: parameters.transactionType,
          });
          [err, data] = await this.http.call(bankConfig);
          break;
        case CONNECTOR_KEY.VERIFY_PAYMENT:
          const verifyConfig =
            this.connectorConfig.getPaymentConfirmationConfig({
              tokenIdentity: parameters.tokenIdentity,
              retrievalReferenceNumber: parameters.retrievalReferenceNumber,
              systemTraceAuditNumber: parameters.systemTraceAuditNumber,
            });
          // [err, data] = await this.http.call(verifyConfig)
          break;
      }
    }
    if (err) {
  ...
  async makeMultipleStagesActive(processInstanceId, setValues) {
    ...
    const response = await this.processInstanceRepositoryImpl.findOneAndUpdate(
      condition,
      setValues,
    );
    for (const stage of response.stages) {
      if (
        ...
        this.logger.debug(
          `${processInstanceId} - [makeMultipleStagesActive Stage Active] ${stage._id}:${stage.name}:${stage.subType}`,
        );
   ...
   */
  async makeStageActive(processInstance, task, userId?: string) {
    this.logger.debug(
      `${processInstance.name} - [In function makeStageActive] ${task.name}:${task.status}`,
    );

    if (task && processInstance) {
      //todo check this
      const valueReplacedProperties = await this.parseAndReplaceValue(
      ...
      let setValues = {
        ...
        ...(task.subType === Constants.STAGE_SUB_TYPES.TIMER && {
          'stages.$.expToCompleteAt': now + task.estimatedTimeDuration,
        }),
      };
      if (task.status !== Constants.STAGE_STATUSES.WAITING) {
        // re-open condition
        task.history.push({
          status: task.status,
      ...
      const stage = response.stages[processInstance._stageIndexJSON[task.key]];

      // let stages = response.stages.filter(stage => stage._id.valueOf() === task._id.valueOf());
      this.logger.debug(
        `${processInstance.name} - [makeStageActive Stage Active] ${task.name}:${task.subType}`,
      );

      await this.onStageActive(response, stage, userId);
    }
  }
  ...
  async makeTaskComplete(processInstance, task, taskData?, assignee?) {
    ...
    [isValid, status] = this.validateCriteria(
      task.criteria || processInstance.criteria,
      task._flags,
    );
    ...
    if (!isValid) {
      if (task.subType !== Constants.STAGE_SUB_TYPES.USER_TASK && task.auto) {
        this.logger.error(
          `Task could not be completed: Parent Instance [${processInstance.name}] -> Task [${task.name}]`,
        );
        return;
      } else {
    ...
    if (taskData?.connector || task.connector) {
      const [err, data, parsedConnector] = await this.callConnector(
        task.connector,
    ...
    if (task.service) {
      if (task.service?.inputs) {
        const inputs = Object.entries(task.service?.inputs);
        const inputValues = await Promise.all(
          inputs.map(async ([key, value]: any) => {
            value = await this.parseAndReplaceValue(value, processInstance);
            return { [key]: value };
          }),
        );
        task.service.inputs = inputValues.reduce((cur, acc) => {
          return { ...acc, ...cur };
        }, {});
      }
      const func = this.appDelegate[task.service.name];
      if (!func) {
        task._flags['_error'] = true;
        task['_error'] = {
      ...
      try {
        processInstance.parameters = await this.appDelegate[task.service.name](
          processInstance,
          task.service?.inputs,
          task.service?.output,
        );
    ...
    this.logger.debug(
      `Task updated: Parent Instance [${processInstance._id}] -> Task [${task._id}:${task.subType}] -> Status [${status}]`,
    );

    const updated = await this.updateFlowFlagStatuses(response);
    const stage = updated.stages.find(
      (stage) => stage._id.valueOf() === task._id.valueOf(),
    );
    if (stage.subType === Constants.STAGE_SUB_TYPES.USER_TASK) {
      // todo task change
      await this.updateUserTask(processInstance._id, stage, assignee);
    }
    if (isValid) {
      // todo task change
      await this.iterateNextStages(
  ...
  async changeTaskAssignee(processInstanceId, task, assignee, watchers) {
    ...
    await this.processInstanceRepositoryImpl.updateOne(condition, setValues);
    this.logger.log(
      `User Tasks updated for the task stage [${JSON.stringify(condition)}]`,
    );
    ...
    this.executorSocket?.to(assignee).emit('newTask', {
      processInstanceId: processInstanceId,
      taskId: task._id,
    });
    this.userTaskRepositoryImpl
      .updateOne(condition1, setValuesTask)
      .then((response) => {
        this.logger.log(
          `User Tasks updated for the task [${JSON.stringify(response)}]`,
        );
      })
      .catch((err) => {
        this.logger.error(err);
  ...
  async changeTaskAssigneeToUser(processInstanceId, task, assignee) {
    const setValuesTask = { assignee };
    const condition1 = { processInstanceId, taskId: task._id };
    this.logger.log(
      `User Tasks updating for condition1 [${JSON.stringify(condition1)}]`,
    );
   ...
   * @returns {promise}
   */
  async createUserTasks(processInstance, task, userId?: string) {
    const taskSubForms = task.subForms
      .map((formName) => {
        const subFormStageIndex = processInstance._stageIndexJSON[formName];
        const subFormFields =
          processInstance.stages[subFormStageIndex].properties;
        const subFormValues =
          processInstance.stages[subFormStageIndex].parameters;
        if (!subFormValues) return null;
        return {
          key: formName,
          name: processInstance.stages[subFormStageIndex].name,
          fields: subFormFields.map((field) => {
            return {
              key: field.key,
              type: field.value.type,
              value: subFormValues[field.key],
    ...
    const taskFields = task?.properties?.map((t) => t.key) || [];
    const otherTaskData = task?.data || [];
    const uniqueTaskData = [...new Set(taskFields.concat(otherTaskData))];
    const taskData = processInstance.processVariables
      .filter((value, index) => {
        const _value = JSON.stringify(value);
        return (
          index ===
          processInstance.processVariables.findIndex((obj) => {
            return JSON.stringify(obj) === _value;
          })
        );
      })
      .filter((v) => uniqueTaskData.includes(v.key))
      .map((variable) => {
    ...
    const stage =
      processInstance.stages[processInstance._stageIndexJSON[task.key]];
    const assignee = await this.valueLocator(processInstance, stage.assignee);
    const userTask = {
      ...
      caseNo: processInstance.caseNo,
      taskId: task._id,
      key: task.key,
      properties: task.properties,
      subForms: taskSubForms,
      data: taskData,
      ...
      expEndDate: moment()
        .add(task.estimatedTimeDuration, 'ms')
        .endOf('day')
        .valueOf(),
      ...
      dueDate: !!task?.dueDate
        ? moment(task.timeActivated)
            .add(ms(task?.dueDate), 'ms') //todo delete after update all definitions
            .valueOf()
        : null,
    ...
    if (!task.assignee && processInstance.assigneeConnector) {
      const connector = JSON.parse(
        JSON.stringify(processInstance.assigneeConnector),
      );
      connector.config['data'] = {};
      this.logger.debug(
        `[${connector.type}] connector called in createUserTasks `,
      );
      ...
      if (userTaskDB) {
        return this.userTaskRepositoryImpl
          .updateOne(
            { _id: userTaskDB._id },
            {
              status: 'todo',
              history: userTask.history,
              data: userTask.data,
              subForms: userTask.subForms,
              assignee: userTask.assignee,
              watchers: userTask.watchers,
            },
          )
          .then((response) => {
            this.logger.log(
              `User Task reopened for the task [${task.name}] in if cond`,
            );
          })
          .catch((err) => {
            this.logger.error(err);
      ...
      if (userTaskDB) {
        return this.userTaskRepositoryImpl
          ...
          .then((response) => {
            this.logger.log(
              `User Task reopened for the task [${task.name}] in else cond`,
            );
          })
          .catch((err) => {
            this.logger.error(err);
   ...
   * @param assignee {string} - assignee
   */
  async updateUserTask(processInstanceId, task, assignee) {
    const setValues = {
    ...
    try {
      await this.userTaskRepositoryImpl.updateOne(
        { processInstanceId, taskId: task._id },
        setValues,
      );
   ...
   * @param task {object} - activity stage object
   */
  async onStageActive(processInstance, task, userId?: string) {
    // activities when task becomes active
    this.logger.debug(
      `[Stage Active] ${processInstance.name}:${task.name}:${task.subType}`,
    );
  ...
  async startFlow(processInstance, payload) {
    ...
    if (processInstance.isParallel) {
      ...
      const activeStages = processInstance.stages.map((_stage) => {
        ...
        if (
          stage.type === Constants.STAGE_TYPES.ACTIVITY &&
          stage.status === Constants.STAGE_STATUSES.WAITING &&
          !skipStages.includes(stage.key)
        ) {
          stage.status = Constants.STAGE_STATUSES.ACTIVE;
          stage.timeActivated = Date.now();
  ...
  async makeFlowEnd(processInstance, task, taskData?, assignee?) {
    ...
    setValues[`stages.${processInstance._stageIndexJSON}.status`] =
      Constants.STAGE_STATUSES.COMPLETED;
    const activeStage = processInstance.stages.find(
      (obj) => obj.status === Constants.STAGE_STATUSES.ACTIVE,
    );
    if (activeStage) {
      const setValues = {
        'stages.$.status': Constants.STAGE_STATUSES.COMPLETED,
      ...
      const [isValid, status] = this.validateCriteria(
        processInstance.criteria,
        processInstance._flags,
      );
      if (isValid) {
        const setValues = {
          status: Constants.STAGE_STATUSES.COMPLETED,
        ...
        // todo task change
        await this.onFlowEnd(response, assignee);
        return;
      }
    ...
    try {
      ...
      if (processInstance.parentTaskId) {
        const parentProcessInstance =
          await this.processInstanceRepositoryImpl.findOne(
            {
              _id: processInstance.parentProcessInstanceId,
              'stages._id': processInstance.parentTaskId,
            },
            { _stageIndexJSON: 1, 'stages.$': 1 },
          );
        if (parentProcessInstance) {
          const compoundTask = JSON.parse(
            JSON.stringify(parentProcessInstance.stages[0]),
          );
   ...
   * @returns {undefined}
   */
  async goToNextStage(inProcessInstance, nextStageKey, userId?: string) {
    const getInstanceStart = await this.processInstanceRepositoryImpl.findOne({
      _id: inProcessInstance._id,
    });
    const processInstance = await this.updateFlowFlagStatuses(getInstanceStart);
    if (nextStageKey) {
      const nextStage =
        processInstance.stages[processInstance._stageIndexJSON[nextStageKey]];
      switch (nextStage?.type) {
        case Constants.STAGE_TYPES.EVENT:
          switch (nextStage.subType) {
            case Constants.STAGE_SUB_TYPES.END:
              ...
              // if (isValid) {
              // todo task change
              await this.makeFlowEnd(processInstance, nextStage, {}, userId);
              // } else {
        ...
        case Constants.STAGE_TYPES.ACTIVITY:
          switch (nextStage.subType) {
            case Constants.STAGE_SUB_TYPES.USER_TASK:
              // todo task change
              await this.makeStageActive(processInstance, nextStage, userId);
              break;
            case Constants.STAGE_SUB_TYPES.SYSTEM_TASK:
            case Constants.STAGE_SUB_TYPES.SERVICE_TASK:
            case Constants.STAGE_SUB_TYPES.TASK:
              if (nextStage.auto) {
                // mark current task as complete
                // todo task change
                await this.makeTaskComplete(processInstance, nextStage);
              } else {
                // todo task change
                await this.makeStageActive(processInstance, nextStage, userId);
              }
        ...
        case Constants.STAGE_TYPES.GATEWAY:
          // await this.makeStageActive(processInstance, nextStage);
          await this.gatewayHandler(processInstance, nextStage, userId);
          break;
        default:
          const [isValid, status] = this.validateCriteria(
            processInstance.criteria,
          ...
          if (isValid) {
            // todo task change
            await this.makeFlowEnd(processInstance, nextStage);
          } else {
            // todo task change
            await this.makeStageActive(processInstance, nextStage);
          }
  ...
  async updateFlowFlagStatuses(processInstance) {
    ...
    return await this.processInstanceRepositoryImpl.findOneAndUpdate(
      condition,
      setValues,
    );
  }
   ...
   * @param processInstance {object} - process instance object
   */
  async makeFlowHold(processInstance, holdBy?, taskData?: CompleteTaskBody) {
    const condition = {
      rootProcessInstanceId: processInstance.rootProcessInstanceId,
      status: Constants.STAGE_STATUSES.ACTIVE,
    };
    ...
    const response = await this.processInstanceRepositoryImpl.updateMany(
      condition,
      setValues,
    );

    // hold all todo user task
    const updateTask = await this.userTaskRepositoryImpl.updateMany(
      {
        rootProcessInstanceId: processInstance.rootProcessInstanceId,
        status: 'todo',
      },
   ...
   * @param processInstance {object} - process instance object
   * @param taskData {object} - task object
   */
  async makeFlowCancel(
    processInstance,
    taskData?: any,
    userId?: string,
    ...
    let setValues = {};
    if (!!taskData) {
      const task = processInstance.stages.find((obj) => {
        if (taskData.taskId) {
      ...
      setValues = { parameters: { ...parameters, ...taskData?.parameters } };
    }
    const condition = {
      rootProcessInstanceId: processInstance.rootProcessInstanceId,
    ...
    setValues = {
      ...
      cancelledBy: userId,
      reason: taskData ? taskData.reason : cancelProcessDto.reason,
      description: taskData
        ? taskData.description
        : cancelProcessDto.description,
    };
    const updateInstance = this.processInstanceRepositoryImpl.updateMany(
      condition,
      setValues,
    );

    // cancel all user task, can use cancell status
    const updateTask = this.userTaskRepositoryImpl.updateMany(condition, {
      status: 'done',
    });

    await Promise.all([updateInstance, updateTask]);
  }
   ...
   * @param processInstance {object} - process instance object
   */
  async makeFlowResume(processInstance, resumedBy?) {
    const condition = {
      rootProcessInstanceId: processInstance.rootProcessInstanceId,
      status: Constants.STAGE_STATUSES.ON_HOLD,
    };
    ...
    // resume all hold user task
    const updateTask = await this.userTaskRepositoryImpl.updateMany(condition, {
      status: 'todo',
    });
  }
  ...
  async saveParams(processInstance, taskData, assignee?) {
    if (taskData?.connector) {
      ...
      this.logger.debug(
        `[${connector.type}] connector called in saveParams `,
        connector,
      );
    ...
    if (taskData.taskId) {
      const _taskDetails = processInstance.stages.find(
        (obj) => obj._id.toString() === taskData.taskId,
      );
      // const setValues = { ..._taskDetails?.parameters, ...taskData.parameters };

      // await this.processInstanceRepositoryImpl.updateOne(
      //   { _id: processInstance._id, 'stages._id': taskData.taskId },
      //   { 'stages.$.parameters': setValues },
      // );
      ...
      // console.log(_taskDetails?.data);

      const taskFields = _taskDetails?.properties?.map((t) => t.key) || [];
      const otherTaskData = _taskDetails?.data || [];
      const uniqueTaskData = [...new Set(taskFields.concat(otherTaskData))];

      const data = instance.processVariables
        .filter((v) => uniqueTaskData.includes(v.key))
        .map((variable) => {
          const value = _.get(instance, `parameters.${variable.key}`);
          return {
            ...variable,
            value: value,
          };
      ...
      await this.userTaskRepositoryImpl.updateOne(
        ...
        { updatedBy: { userId: assignee }, data },
      );
    } else {
      const setValues = {
        parameters: { ...processInstance.parameters, ...taskData.parameters },
