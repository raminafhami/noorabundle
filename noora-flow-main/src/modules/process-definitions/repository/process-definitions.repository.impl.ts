...
@Injectable()
export class ProcessDefinitionRepositoryImpl
  extends BaseRepositoryImpl<ProcessDefinitionDocument>
  implements ProcessDefinitionRepository
{
  constructor(
    @InjectModel(ProcessDefinition.name)
    protected ProcessDefinitionModel: Model<ProcessDefinitionDocument>,
  ) {
    super(ProcessDefinitionModel);
  }

  create(
    object: CreateProcessDefinitionDto,
  ...
  update(
    condition: any,
    object: UpdateProcessDefinitionDto,
  ): Promise<ProcessDefinitionDocument> {
    return this.ProcessDefinitionModel.findOneAndUpdate(condition, object, {
      new: true,
    }).exec();
  }
